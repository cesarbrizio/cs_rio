...
export function buildFavelaForceSummaryLines(favela: TerritoryFavelaSummary): string[] {
  ...
  return [
    `Soldados ${favela.soldiers.active}/${favela.soldiers.max} · ocupacao ${Math.round(favela.soldiers.occupancyPercent)}%`,
    [`Bandidos ${favela.bandits.active}/${favela.bandits.targetActive}`, ...banditTail].join(' · '),
  ];
}
...
export function buildFavelaAlertLines(favela: TerritoryFavelaSummary): string[] {
  ...
  if (favela.satisfactionProfile.tier === 'collapsed') {
    lines.push('Moradores entraram em colapso e a favela esta perto de quebrar.');
  } else if (favela.satisfactionProfile.tier === 'critical') {
    lines.push('Satisfacao critica: a chance de X9 e de perda de receita esta alta.');
  }

  if (favela.x9?.status === 'warning') {
    lines.push('Tem X9 soprando e a incursao pode cair a qualquer momento.');
  } else if (favela.x9?.status === 'pending_desenrolo') {
    lines.push('Membros presos aguardam desenrolo ou vao cumprir a janela completa.');
  }

  if (favela.propina?.status === 'severe') {
    lines.push('Arrego atrasado esta cortando a receita territorial.');
  } else if (favela.propina?.status === 'state_takeover') {
    lines.push('O Estado ja tomou a favela por inadimplencia prolongada.');
  }

  if (favela.war?.status === 'preparing' || favela.war?.status === 'active') {
    lines.push('A favela esta no ciclo quente de guerra e exige resposta rapida.');
  }
...
export function buildTerritoryHeadlineStats(
  ...
  return {
    ...
    x9ActiveFavelas: favelas.filter(
      (favela) => favela.x9 !== null && favela.x9.status !== 'resolved',
    ).length,
  };
}

export function formatTerritoryCountdown(value: string | null, nowMs: number): string | null {
  if (!value) {
    return null;
  }
