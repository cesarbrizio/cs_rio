...
export function buildCombatTargets(input: {
  ...
  realtimePlayers: RealtimePlayerSnapshot[];
}): CombatTargetOption[] {
...
export function buildAmbushParticipantOptions(input: {
  ...
  return input.members
    ...
    .map((member) => {
      ...
      return {
        disabledReason: isEligible ? null : 'Só soldado ou superior entra na emboscada.',
        id: member.id,
        isEligible,
...
export function buildCombatResultHighlights(result: CombatResult): string[] {
  ...
  if (result.loot && result.loot.amount > 0) {
    lines.push(`Espólio puxado: ${formatCombatCurrency(result.loot.amount)} (${result.loot.percentage}% do bolso).`);
  }
  ...
  if (result.mode === 'ambush') {
    lines.push(
      `Bonde com ${result.attackers.length} membros · coordenação x${result.coordinationMultiplier.toFixed(2)}.`,
    );
  }
