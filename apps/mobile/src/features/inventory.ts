...
export type InventoryActionKind = 'equip' | 'repair' | 'unequip' | 'use';
...
export function buildInventoryBenefitLines(item: PlayerInventoryItem): string[] {
  ...
  if (equipment?.slot === 'weapon' && typeof equipment.power === 'number') {
    return [
      ...
      `Combate direto soma +${equipment.power} ao seu poder total de confronto.`,
  ...
  if (equipment?.slot === 'vest' && typeof equipment.defense === 'number') {
    return [
      ...
      `Combate direto soma +${equipment.defense} de protecao efetiva no confronto.`,
