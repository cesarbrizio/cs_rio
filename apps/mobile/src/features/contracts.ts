...
import { formatCombatCurrency, resolveCombatTierLabel } from './combat';
...
export function buildContractExecutionHighlights(result: PvpContractExecutionResponse): string[] {
  ...
  if (result.loot?.amount) {
    lines.push(`Espólio puxado: ${formatCombatCurrency(result.loot.amount)}.`);
  }
  ...
  lines.push(result.targetNotified ? 'O alvo ficou avisado após a tentativa.' : 'O alvo não foi alertado antes do fim.');
