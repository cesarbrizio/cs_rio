...
export function resolveContactOriginLabel(origin: PlayerContactSummary['origin']): string {
  return origin === 'same_faction' ? 'Mesmo bonde' : 'Manual';
}

export function resolveContactTypeLabel(type: PlayerContactSummary['type']): string {
...
function buildRosterEntry(
  ...
  const preview = thread?.lastMessage
    ? `${thread.lastMessage.senderId === contact.contactId ? contact.nickname : 'Você'}: ${thread.lastMessage.message}`
