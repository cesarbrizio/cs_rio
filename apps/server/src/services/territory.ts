...
function buildTerritoryConquestLogDescription(
...
function mapTerritoryX9EventRow(row: {
  desenroloAttemptedAt: Date | null;
...
function parseTerritoryX9SoldierImpactJson(value: unknown): TerritoryX9SoldierImpactRecord[] {
...
function buildTerritoryX9IncursionImpacts(input: {
  ...
  const totalSoldiers = input.exposure.soldierTargets.reduce((sum, target) => sum + target.soldiersCount, 0);
  const soldiersArrested =
    totalSoldiers > 0 ? Math.min(totalSoldiers, 1 + Math.floor(input.random() * 3)) : 0;
  const soldierImpacts = allocateX9SoldierArrests(input.exposure.soldierTargets, soldiersArrested);
  const weaponsLost =
    totalSoldiers > 0 ? Math.max(soldiersArrested, resolveX9QuantityLoss(totalSoldiers, weaponsLossRate)) : 0;
  const baseMoneyCost = roundCurrency(
    Math.max(5000, moneyLost * 0.45 + drugsLost * 30 + weaponsLost * 120 + soldiersArrested * 2500),
  );
...
function canPlayerAttemptX9Desenrolo(
  ...
  return (
    commandRanks.includes(player.rank as FactionRank) ||
    player.vocation === VocationType.Politico
  );
}
...
function mapTerritoryFactionWarRow(row: {
  attackerFactionId: string;
...
function serializeTerritoryFactionWarPreparation(
...
function serializeTerritoryFactionWarRound(
...
function parseTerritoryFactionWarRoundsJson(
  ...
  return value
    ...
    .map((entry) => ({
      ...
      attackerNerveLoss: Number(entry.attackerNerveLoss ?? 0),
      attackerPower: Number(entry.attackerPower ?? 0),
      attackerStaminaLoss: Number(entry.attackerStaminaLoss ?? 0),
      defenderHpLoss: Number(entry.defenderHpLoss ?? 0),
      defenderNerveLoss: Number(entry.defenderNerveLoss ?? 0),
      defenderPower: Number(entry.defenderPower ?? 0),
      defenderStaminaLoss: Number(entry.defenderStaminaLoss ?? 0),
