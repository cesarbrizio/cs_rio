...
const STAMINA_RESOURCE_KEY = 'stamina';

export interface StaminaSyncContext {
  baselineAt?: Date | string | null;
  max?: number;
  recoveryPerHour?: number;
}

export interface StaminaSyncResult {
  changed: boolean;
  key: string;
  nextValue: number;
...
export class StaminaSystem {
  ...
  async sync(
    playerId: string,
    currentValue: number,
    context: StaminaSyncContext = {},
  ): Promise<StaminaSyncResult> {
    const now = this.now();
    const key = buildTimedResourceKey(STAMINA_RESOURCE_KEY, playerId);
