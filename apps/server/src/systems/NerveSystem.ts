...
const NERVE_RESOURCE_KEY = 'nerve';

export interface NerveSyncContext {
  baselineAt?: Date | string | null;
  max?: number;
}

export interface NerveSyncResult {
  changed: boolean;
  key: string;
  nextValue: number;
...
export interface NerveSystemOptions {
...
export class NerveSystem {
  ...
  constructor(options: NerveSystemOptions = {}) {
  ...
  async sync(playerId: string, currentValue: number, context: NerveSyncContext = {}): Promise<NerveSyncResult> {
    const now = this.now();
    const key = buildTimedResourceKey(NERVE_RESOURCE_KEY, playerId);
    ...
    const recoveredPoints = Math.floor(Math.max(0, now - currentState.lastSyncAt) / NERVE_INTERVAL_MS);
    const nextValue = clamp(previousValue + recoveredPoints, 0, max);
    const lastSyncAt =
      nextValue >= max
        ? now
        : currentState.lastSyncAt + recoveredPoints * NERVE_INTERVAL_MS;
