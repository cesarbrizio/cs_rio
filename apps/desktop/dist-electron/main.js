...
const DEFAULT_DESKTOP_RESOLUTION_PRESET = DESKTOP_RESOLUTION_PRESETS[0];
function resolveResolutionPreset(presetId) {
  return DESKTOP_RESOLUTION_PRESETS.find((preset) => preset.id === presetId) ?? DEFAULT_DESKTOP_RESOLUTION_PRESET;
}
...
const DESKTOP_RUNTIME_STORAGE_KEY = "cs_rio_desktop_runtime_v1";
const desktopStore = new ElectronStore({
  name: "cs-rio-desktop"
});
...
function patchRuntimeSnapshot(patcher) {
  const current = readRuntimeSnapshot() ?? {};
  const next2 = patcher(current);
  desktopStore.set(DESKTOP_RUNTIME_STORAGE_KEY, JSON.stringify(next2));
}
...
function applyShellState(window, nextState) {
  shellState = {
    ...nextState
  };
  persistShellState();
...
function shouldShowNativeNotifications() {
...
function loadInitialShellState() {
  const snapshot = readRuntimeSnapshot();
  const notificationSettings = snapshot?.notificationSettings;
  const shellSettings = snapshot?.shellSettings;
  return {
    displayMode: shellSettings?.displayMode === "borderless" || shellSettings?.displayMode === "fullscreen" || shellSettings?.displayMode === "windowed" ? shellSettings.displayMode : DEFAULT_SHELL_STATE.displayMode,
    minimizeToTray: shellSettings?.minimizeToTray ?? DEFAULT_SHELL_STATE.minimizeToTray,
    notificationsEnabled: notificationSettings?.enabled ?? DEFAULT_SHELL_STATE.notificationsEnabled,
    resolutionPresetId: typeof shellSettings?.resolutionPresetId === "string" ? shellSettings.resolutionPresetId : DEFAULT_SHELL_STATE.resolutionPresetId
  };
}
...
function cancelScheduledNotification(id2) {
  const timeout = scheduledNotifications.get(id2);
  if (!timeout) {
    return;
  }
  clearTimeout(timeout);
  scheduledNotifications.delete(id2);
}
function registerNotifyIpcHandlers() {
  electron.ipcMain.removeHandler(NOTIFY_HAS_PERMISSION_CHANNEL);
  electron.ipcMain.removeHandler(NOTIFY_SHOW_CHANNEL);
  electron.ipcMain.removeHandler(NOTIFY_SCHEDULE_CHANNEL);
  electron.ipcMain.removeHandler(NOTIFY_CANCEL_CHANNEL);
  electron.ipcMain.removeHandler(NOTIFY_CANCEL_ALL_CHANNEL);
  electron.ipcMain.removeHandler(NOTIFY_PERMISSION_CHANNEL);
  electron.ipcMain.handle(NOTIFY_HAS_PERMISSION_CHANNEL, async () => electron.Notification.isSupported());
  electron.ipcMain.handle(NOTIFY_SHOW_CHANNEL, async (_event, title2, body) => {
    if (!electron.Notification.isSupported() || !shouldShowNativeNotifications()) {
      return false;
    }
  ...
  electron.ipcMain.handle(NOTIFY_CANCEL_CHANNEL, async (_event, id2) => {
    cancelScheduledNotification(id2);
  });
  electron.ipcMain.handle(NOTIFY_CANCEL_ALL_CHANNEL, async () => {
    for (const id2 of scheduledNotifications.keys()) {
      cancelScheduledNotification(id2);
    }
  });
  electron.ipcMain.handle(NOTIFY_PERMISSION_CHANNEL, async () => electron.Notification.isSupported());
}
...
const SHELL_NOTIFICATIONS_CHANGED_EVENT = "shell:notifications-enabled-changed";
function emitNotificationsEnabledChanged(enabled) {
  for (const window of electron.BrowserWindow.getAllWindows()) {
    window.webContents.send(SHELL_NOTIFICATIONS_CHANGED_EVENT, enabled);
  }
}
function registerShellIpcHandlers({
  requestQuit: requestQuit2,
  resolveMainWindow,
  trayManager: trayManager2
}) {
  electron.ipcMain.removeHandler(SHELL_SYNC_SETTINGS_CHANNEL);
  electron.ipcMain.removeHandler(SHELL_TOGGLE_FULLSCREEN_CHANNEL);
  electron.ipcMain.removeAllListeners(SHELL_QUIT_CHANNEL);
  electron.ipcMain.handle(SHELL_SYNC_SETTINGS_CHANNEL, async (_event, settings) => {
    const window = resolveMainWindow();
    if (window) {
      applyShellState(window, settings);
    }
    trayManager2.setNotificationsEnabled(settings.notificationsEnabled);
  });
  electron.ipcMain.handle(SHELL_TOGGLE_FULLSCREEN_CHANNEL, async () => {
    const window = resolveMainWindow();
    if (!window) {
      return false;
    }
  ...
  electron.ipcMain.on(SHELL_QUIT_CHANNEL, () => {
    requestQuit2();
    electron.app.quit();
  });
}
...
function registerStorageIpcHandlers() {
  electron.ipcMain.removeHandler(STORAGE_GET_CHANNEL);
  electron.ipcMain.removeHandler(STORAGE_SET_CHANNEL);
  electron.ipcMain.removeHandler(STORAGE_REMOVE_CHANNEL);
  electron.ipcMain.handle(STORAGE_GET_CHANNEL, async (_event, key) => {
    return desktopStore.get(key) ?? null;
  });
  electron.ipcMain.handle(STORAGE_SET_CHANNEL, async (_event, key, value) => {
    desktopStore.set(key, value);
  });
  electron.ipcMain.handle(STORAGE_REMOVE_CHANNEL, async (_event, key) => {
...
function resolveSenderWindow(webContentsId) {
  return electron.BrowserWindow.getAllWindows().find(
    (candidateWindow) => candidateWindow.webContents.id === webContentsId
  ) ?? null;
}
function registerWindowIpcHandlers() {
  electron.ipcMain.removeAllListeners(WINDOW_MINIMIZE_CHANNEL);
  electron.ipcMain.removeAllListeners(WINDOW_MAXIMIZE_CHANNEL);
  electron.ipcMain.removeAllListeners(WINDOW_CLOSE_CHANNEL);
  electron.ipcMain.removeAllListeners(WINDOW_FULLSCREEN_CHANNEL);
  electron.ipcMain.on(WINDOW_MINIMIZE_CHANNEL, (event) => {
  ...
  electron.ipcMain.on(WINDOW_MAXIMIZE_CHANNEL, (event) => {
    const window = resolveSenderWindow(event.sender.id);
    if (!window) {
      return;
    }
...
class DesktopTrayManager {
  ...
  notifyHiddenToTray() {
    if (!this.notificationsEnabled || !electron.Notification.isSupported()) {
      return;
    }
    new electron.Notification({
...
function resolveRendererAssetPath(requestUrl) {
  ...
  const normalizedRequestPath = url.pathname && url.pathname !== "/" ? url.pathname : "/index.html";
  const sanitizedPath = path.normalize(decodeURIComponent(normalizedRequestPath)).replace(/^(\.\.(\/|\\|$))+/, "").replace(/^[/\\]+/, "");
...
async function registerCsrioProtocol() {
  if (electron.protocol.isProtocolHandled("csrio")) {
    return;
  }
  await electron.protocol.handle("csrio", (request) => {
    const assetPath = resolveRendererAssetPath(request.url);
    return electron.net.fetch(node_url.pathToFileURL(assetPath).toString());
  });
}
function createMainWindow() {
  const nextWindow = new electron.BrowserWindow({
